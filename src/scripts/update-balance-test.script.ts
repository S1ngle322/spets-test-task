...
async function sendRequests(): Promise<void> {
  ...
  for (let i = 0; i < requestsCount; i++) {
    requests.push(
      await axios.post<IUpdateBalanceResponse>(URL, { userId, amount })
  ...
  const successful: number = results.filter(r =>
    !(r instanceof Error) && r.data?.success
  ).length

  const failed: number = results.filter(r =>
    r instanceof Error || !r.data?.success
  ).length;
